...
if __name__ == '__main__':
    ...
    for j, stepsize in enumerate(stepsizeArray):
        ...
        for k, density in enumerate(densityArray):
            ...
            sample(NB_PARTICLES, NB_PARTICLES_PER_PROC, EnergyDummy, stepsize, material)
            if rank == 0:
                ...
                lut[j, k, :] = temp.mean(), TEEx.scoreMatrix.mean(), TEEy.scoreMatrix.mean(), TEEz.scoreMatrix.mean(), TEEx.scoreMatrix.var(), TEEy.scoreMatrix.var(), TEEz.scoreMatrix.var()
        t4 = time.process_time()
        if rank == 0:
            print(f'{round(100*(j+1)/stepsizeArray.size, 3)}% completed. Last section took {(t4-t3)/60:2e} minutes.')
